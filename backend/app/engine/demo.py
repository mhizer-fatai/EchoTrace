    ...
    if _is_question(content):
        ...
        if query_res.status == "ANSWERED":
            ...
            if query_res.history:
                ...
                reply += f"\n\n⏳ **Superseded History:**\n{history_lines}"
        elif query_res.status == "CONFLICT":
            conflict_lines = "\n".join(
                f"- `{c.value}` (from `{c.session_id}`)" for c in query_res.evidence
            )
            reply = (
                f"⚠️ **Memory Conflict Detected**\n\n"
                f"Two active memories disagree about your "
                f"**{query_res.property_name.replace('_', ' ')}**:\n{conflict_lines}\n\n"
                f"🛡️ **Status:** `CONFLICT`\n"
                f"EchoTrace abstains rather than guess. State an explicit change — "
                f"e.g. \"I moved my trip to ...\" — to resolve."
            )
        else:
...
def seed_conflict() -> Dict[str, Any]:
    """Simulate a second agent session recording a trip value that contradicts
    the currently active one WITHOUT an explicit update. The result is two
    VALID facts for the same property — a conflict EchoTrace will abstain on
    until the user states an explicit change."""
    existing = graph_client.get_session_graph(DEMO_SESSION_ID).get("nodes", [])
    if any(node.get("id") == "demo_fact_conflict_july" for node in existing):
        return _conflict_status("already_seeded")

    now = datetime.now(timezone.utc)
    msg = MessageNode(
        id="demo_msg_conflict_july",
        label="Session 05: trip claimed July",
        user_id=DEMO_USER_ID,
        source_session_id="session_05",
        message_index=0,
        role="user",
        content="My trip is in July.",
        session_id=DEMO_SESSION_ID,
        created_at=now,
        valid_from=now,
    )
    fact = FactNode(
        id="demo_fact_conflict_july",
        label="Trip: July (second source)",
        entity=DEMO_USER_ID,
        property_name="trip",
        property_value="July",
        status=FactStatus.VALID,
        source_agent_id="demo_agent_flight_agent",
        session_id=DEMO_SESSION_ID,
        created_at=now,
        valid_from=now,
        metadata={
            "source_session_id": "session_05",
            "message_index": 0,
            "quote": "My trip is in July.",
        },
    )
    graph_client.add_node(msg)
    graph_client.add_node(fact)
    graph_client.add_edge(GraphEdge(
        id="demo_edge_conflict_july_source",
        source_id=fact.id,
        target_id=msg.id,
        edge_type=EdgeType.SUPPORTED_BY,
        created_at=now,
    ))
    return _conflict_status("conflict_seeded")


def _conflict_status(status: str) -> Dict[str, Any]:
    graph = graph_client.get_session_graph(DEMO_SESSION_ID)
    active_trip = [
        node for node in graph.get("nodes", [])
        if node.get("kind") == "FACT"
        and node.get("property_name") == "trip"
        and node.get("status") == FactStatus.VALID.value
        and node.get("entity") == DEMO_USER_ID
    ]
    return {
        ...
        "user_id": DEMO_USER_ID,
        "session_id": DEMO_SESSION_ID,
        "conflicting_fact_id": "demo_fact_conflict_july",
        "conflicting_value": "July",
        "conflicting_source_session": "session_05",
        "active_trip_facts": [_fact_summary(node["id"]) for node in active_trip],
        "hydradb_connected": graph_client.connected_to_hydradb,
        "engine_mode": "HydraDB Bolt" if graph_client.connected_to_hydradb else "Internal Graph Engine",
    }
